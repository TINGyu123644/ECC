...
After listing review findings, you MUST verify them by running the project's
actual toolchain. This is non-optional — silencing the loop is grounds for
failing the review.

### When to verify

Any finding that depends on runtime/type/import behavior (not pure style) — for
example "this will throw at runtime", "this type will not infer", "this import
path does not resolve", "this test will fail on CI".

### Verification procedure

1. Identify the smallest set of files to verify the claim.
2. Run, in order, until exit code 0 on all of them:
   - `node --check <file>` or the project's type-check (`tsc --noEmit`,
     `pnpm typecheck`, `mvn compile`, `go build ./...`, etc.)
   - The project's linter (`eslint`, `ruff`, `golangci-lint`, etc.)
   - The project's test command for files you touched
3. If any command exits non-zero:
   - **Re-read the failing line in context**
   - Re-classify the original finding: was it real, partially wrong, or stale?
   - Drop findings that the run contradicts (a "bug" that the type-checker
     proves cannot occur is not a bug)
   - Upgrade severity only when the run proves a worse impact than you first
     stated (e.g., you said LOW runtime risk, the run shows the call site
     crashes on cold start → MEDIUM)
4. Do not mark any review as `APPROVE` until at least one verification command
   has been run on every file touched by the diff. A review that runs zero
   verifications is incomplete.

### Hard rule: never claim a fix is required without seeing the tool output.

If the diff is small enough that no command applies, say so explicitly in the
review summary ("No applicable verification command — review based on reading
only") rather than implying confidence you did not earn.

### Loop until consistent

If the verification output disagrees with your findings, loop:

- State the disagreement
- Re-examine the file
- If the file is right, **your finding is wrong** — drop or correct it
- If the file is wrong, your finding is a real bug — keep it and fix
- Never trust the LLM prior over the deterministic tool
